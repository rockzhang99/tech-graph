...
| 越界自动修复 | 节点超出容器边界（`CONTAINER_GUTTER`）时自动收回（优先平移、必要时才缩小），并把修正后的坐标写回编辑器 |
| 导出 | SVG 源文件、PNG（960–3840px）、GIF 语义动效（5.75s / 115 帧）、离线交互 HTML |
| 布局报告 | 视觉风格、节点连线规模、构图评分（实测值对照上限，如 `total_bends 4 / 限 8`）、问题清单 |
| 外部格式导入 | 第三方绘图工具导出的 `elements` 图描述（`Group` / `Shape` / `Icon` / `Relationship`）自动转换为本工具的图规格后渲染 |
| 其他 | 13 个内置示例、JSON 编辑器、拖拽导入 `.svg` / `.json`（自动识别 UTF-8 / UTF-16 / GBK）、画布缩放与三种背景、`Ctrl+Enter` 快捷渲染 |
...
│   ├── convert.py           # 外部 elements 图描述 → 本工具图规格
│   ├── requirements.txt
...
- 图规格只识别 `containers` / `nodes` / `arrows` 三个顶层数组，**其余顶层字段静默忽略**。
  粘贴第三方工具的 JSON 若只渲染出标题，基本都是 schema 不匹配；`elements` 格式会自动转换，其他格式需手工改写成上面「图规格 JSON 示例」的形状。
